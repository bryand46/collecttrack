...
const MANUFACTURER_SUGGESTIONS: Record<string, string[]> = {
  ...
  'Sneakers & Footwear': ['Nike', 'Adidas', 'Jordan', 'New Balance', 'Yeezy', 'Converse', 'Vans', 'Reebok', 'Asics', 'Travis Scott'],
...
export default function EditItemPage() {
  ...
  useEffect(() => {
    fetch('/api/profile')
      .then((r) => r.json())
      .then((data) => {
        if (data?.preferredGroups) setPreferredGroups(JSON.parse(data.preferredGroups))
      })
      .catch(() => {})
  }, [])
  ...
  useEffect(() => {
    fetch(`/api/items/${params.id}`)
      .then((res) => res.json())
      .then((data) => {
        const savedManufacturer = data.manufacturer ?? ''
        const suggestions = getManufacturerSuggestions(data.category ?? '')
        // If saved value isn't in the suggestions list, pre-fill as "Other"
        const isKnown = suggestions.includes(savedManufacturer) || savedManufacturer === ''
        setForm({
          name: data.name ?? '',
          category: data.category ?? '',
          manufacturer: isKnown ? savedManufacturer : 'Other',
          description: data.description ?? '',
          condition: data.condition ?? 'Good',
          paidPrice: data.paidPrice != null ? String(data.paidPrice) : '',
          estimatedValue: data.estimatedValue != null ? String(data.estimatedValue) : '',
          notes: data.notes ?? '',
          imageUrl: data.imageUrl ?? '',
        })
        if (!isKnown) setManufacturerOther(savedManufacturer)
        setLoading(false)
      })
  }, [params.id])
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold" style={{ color: '#0F172A' }}>Edit Item</h1>
        <p className="text-sm mt-1" style={{ color: '#64748B' }}>Update the details for this collectible.</p>
      </div>
